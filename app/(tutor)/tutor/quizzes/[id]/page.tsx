...
export default function TutorQuizReviewPage({ params }: { params: { id: string } }) {
  const router = useRouter()
  const [toast, setToast] = useState('')

  const { data, isLoading } = useQuery(['tutor-quiz-attempt', params.id], async () => {
  ...
  const mutation = useMutation(async (payload: any) => {
    ...
    onSuccess: () => {
      setToast('Attempt successfully reviewed and student notified')
      setTimeout(() => {
        setToast('')
  ...
  if (isLoading) return (
    <TutorLayout title="Reviewing Attempt" breadcrumb="Tutor / Quiz / Review">
  ...
  return (
    <TutorLayout title={`Review Quiz — ${attempt?.student?.name}`} breadcrumb={`Tutor / Quiz / ${attempt?.quiz?.title}`}>
        ...
        <div className="col-lg-8">
          ...
          <div className="card shadow-none border mb-6">
            <div className="card-header border-bottom d-flex align-items-center gap-2">
              ...
              <h5 className="mb-0 fw-bold">Answer Details</h5>
            </div>
                    ...
                    <div key={q.id} className={`p-6 rounded border mb-6 ${isCorrect ? 'border-label-success' : 'border-label-danger'}`}>
                      ...
                      <div className="row g-4">
                        <div className="col-md-6">
                          ...
                          <div className="p-4 rounded bg-white border font-bold">
                        ...
                        <div className="col-md-6">
                          ...
                          <div className="p-4 rounded bg-white border border-dashed text-primary">
                            {q.correctAnswer}
                          </div>
                        </div>
                      </div>
                      ...
                      {q.explanation && (
                        <div className="mt-4 p-4 rounded bg-label-light border-start border-4 border-info small text-body shadow-xs">
        ...
        <div className="col-lg-4">
          <div className="card shadow-none border sticky-top" style={{ top: '100px' }}>
            <div className="card-header border-bottom">
              <h5 className="mb-0 fw-bold">Grading Panel</h5>
            </div>
            <div className="card-body p-6">
              <form onSubmit={handleSubmit((d) => mutation.mutate(d))}>
                <div className="text-center mb-8">
                  ...
                  <h5 className="mb-1 fw-bold">{attempt?.student?.name}</h5>
                ...
                <div className="mb-6 p-4 bg-label-secondary rounded border-0">
                  <div className="d-flex justify-content-between mb-2">
                    ...
                    <span className="fw-bold small">{attempt?.timeTakenSeconds ? `${Math.floor(attempt.timeTakenSeconds / 60)}m ${attempt.timeTakenSeconds % 60}s` : 'N/A'}</span>
                  </div>
                  <div className="d-flex justify-content-between mb-0">
                    <span className="text-muted small">Submitted Date:</span>
                    <span className="fw-bold small">{format(new Date(attempt?.takenAt), 'MMM dd, p')}</span>
                  </div>
                </div>

                <div className="mb-6">
                  <label className="form-label small fw-bold text-uppercase">Final Score Override (%)</label>
                ...
                <div className="mb-8">
                  <label className="form-label small fw-bold text-uppercase">Review Commentary</label>
                  <textarea
                    className={`form-control p-4 fs-6 ${errors.feedback ? 'is-invalid' : ''}`}
                    rows={8}
